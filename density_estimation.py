...
def create_model(args, verbose=False):
    ...
    if verbose:
        ...
        print(f'Parameters={params}, '
              f'NAF/BNAF={NAF_PARAMS[args.dataset][0] / params:.2f}/{NAF_PARAMS[args.dataset][1] / params:.2f}, '
              f'n_dims={args.n_dims}')

    if args.save and not args.load:
        with open(os.path.join(args.load or args.path, 'results.txt'), 'a') as f:
            print(f'Parameters={params}, '
                  f'NAF/BNAF={NAF_PARAMS[args.dataset][0] / params:.2f}/{NAF_PARAMS[args.dataset][1] / params:.2f}, '
                  f'n_dims={args.n_dims}', file=f)
...
def train(model, optimizer, scheduler, data_loader_train, data_loader_valid, data_loader_test, args):
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.epochs):
        ...
        print(f'Epoch {epoch + 1:3}/{args.start_epoch + args.epochs:3} -- '
              f'train_loss: {train_loss.item():4.3f} -- '
              f'validation_loss: {validation_loss.item():4.3f}')
    ...
    print(f'###### Stop training after {epoch + 1} epochs!')
    print(f'Validation loss: {validation_loss.item():4.3f}')
    print(f'Test loss:       {test_loss.item():4.3f}')

    if args.save:
        with open(os.path.join(args.load or args.path, 'results.txt'), 'a') as f:
            print(f'###### Stop training after {epoch + 1} epochs!', file=f)
            print(f'Validation loss: {validation_loss.item():4.3f}', file=f)
            print(f'Test loss:       {test_loss.item():4.3f}', file=f)
...
def main():
    ...
    args.path = os.path.join('checkpoint',
                             f'{args.expname + ("_" if args.expname != "" else "")}{args.dataset}_layers{args.layers}'
                             f'_h{args.hidden_dim}_flows{args.flows}{"_" + args.residual if args.residual else ""}'
                             f'_{str(datetime.datetime.now())[:-7].replace(" ", "-").replace(":", "-")}')
